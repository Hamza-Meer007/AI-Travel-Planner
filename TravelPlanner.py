...
LLM_API_KEY = GROQ_API_KEY if LLM_PROVIDER == "groq" else OPENAI_API_KEY
...
LLM_AUTH_ERROR_MESSAGE = (
    f"{LLM_PROVIDER_DISPLAY_NAME} authentication failed. Update the API key in .env and restart the server."
)
LLM_QUOTA_ERROR_MESSAGE = (
    f"{LLM_PROVIDER_DISPLAY_NAME} quota exceeded. Add billing or increase your quota, then restart the search."
)
...
def initialize_llm():
    ...
    if not LLM_API_KEY:
        raise RuntimeError(
            f"{LLM_PROVIDER_DISPLAY_NAME} API key is missing. Add it to .env and restart the server."
        )
...
def get_llm_rate_limit_message(error: RateLimitError) -> str:
    """Translate provider rate limit errors into actionable user-facing messages."""
...
def is_ai_service_error(message: str) -> bool:
    """Identify responses that represent AI configuration or quota failures."""
    if not message:
        return False
...
async def get_ai_recommendation(data_type, formatted_data):
    ...
    try:
        ...
        logger.exception(f"Error in AI {data_type} analysis: invalid {LLM_PROVIDER_DISPLAY_NAME} credentials")
        return LLM_AUTH_ERROR_MESSAGE
    except RateLimitError as error:
        logger.exception(f"Error in AI {data_type} analysis: {LLM_PROVIDER_DISPLAY_NAME} quota or rate limit issue")
...
async def generate_itinerary(destination, flights_text, hotels_text, check_in_date, check_out_date):
    ...
    try:
        ...
        logger.exception(f"Error generating itinerary: invalid {LLM_PROVIDER_DISPLAY_NAME} credentials")
        return LLM_AUTH_ERROR_MESSAGE
    except RateLimitError as error:
        logger.exception(f"Error generating itinerary: {LLM_PROVIDER_DISPLAY_NAME} quota or rate limit issue")
        return get_llm_rate_limit_message(error)
    except Exception as e:
...
async def complete_travel_search(flight_request: FlightRequest, hotel_request: Optional[HotelRequest] = None):
    ...
    try:
        ...
        itinerary = ""
        ai_service_error = (
            is_ai_service_error(flight_results.ai_flight_recommendation)
        ...
        if flight_results.flights and hotel_results.hotels and not ai_service_error:
            ...
            itinerary = (
                LLM_QUOTA_ERROR_MESSAGE
                if LLM_QUOTA_ERROR_MESSAGE in {
                    flight_results.ai_flight_recommendation,
                    hotel_results.ai_hotel_recommendation,
                }
                else LLM_RATE_LIMIT_ERROR_MESSAGE
                if LLM_RATE_LIMIT_ERROR_MESSAGE in {
                    flight_results.ai_flight_recommendation,
                    hotel_results.ai_hotel_recommendation,
                }
                else LLM_AUTH_ERROR_MESSAGE
            )
